...
| 5 | **Spec Preview Node** | Tree Item (Child) | Child node under a challenge. `label` = "Spec Preview", `iconPath` = `$(book)`. Clicking triggers `topcoder.openSpec` command which opens the Spec Webview (WF2). `command` property set on the `TreeItem`. | `TreeItem.command` | A | WF1, WF2 |
...
| 7 | **Attachments Node** | Tree Item (Child) | Child node showing "Attachments (3)". Count from the challenge detail API. Clicking opens a quick-pick list of downloadable files. Each selection triggers `vscode.env.openExternal()` with the attachment URL. | `TreeItem.command`, `vscode.window.showQuickPick()` | A | WF1 |
| 8 | **Forum Posts Node** | Tree Item (Child) | Child node showing "Forum Posts (12)". Count from the discussions API. Clicking opens the Forum & Timeline webview (WF6). Badge updates on new posts (polling). | `TreeItem`, `TreeItem.description` | C | WF1, WF6 |
...
| 33 | **Open Spec** | Click "Spec Preview" child node | Creates or reveals the Spec Webview panel. Populates with rendered markdown from the challenge `description` field. | Spec Preview Node (#5), Spec Webview (#12) | A |
...
| 44 | **View Forum** | Click "Forum Posts" child node in tree | Opens/reveals the Forum & Timeline webview (WF6). Fetches `GET /v5/challenge-discussions`. Renders posts as cards. Starts auto-poll if enabled. | Forum Posts Node (#8), Forum Webview (#25) | C |
...
| 50 | **Challenge Object** | API Response | JSON object from `GET /v6/challenges/{id}`. Key fields: `id`, `name`, `description` (markdown spec), `status` (Active/Upcoming/Completed), `phases[]`, `prizeSets[]`, `tags[]`, `attachments[]`, `numOfRegistrants`, `numOfSubmissions`. Cached in `globalState` with a 5-minute TTL. | `GET /v6/challenges/{id}` | WF1, WF2, WF3 |
